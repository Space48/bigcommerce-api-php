...
use Space48\Bigcommerce\Requests\Videos\GetProductVideo;
...
class Videos extends Resource
{
	 ...
	 * @param string $excludeFields Fields to exclude, in a comma-separated list. The specified fields will be excluded from a response. The ID cannot be excluded.
	 */
	public function getProductVideo(int $productId, int $id, ?string $includeFields, ?string $excludeFields): Response
	{
		return $this->connector->send(new GetProductVideo($productId, $id, $includeFields, $excludeFields));
	}
	...
	{
		return $this->connector->send(new DeleteProductVideo($productId, $id));
	}
}
